// @ts-nocheck — vendored unmodified; upstream wasn't written against this
// repo's noUncheckedIndexedAccess/exactOptionalPropertyTypes strictness.
// Tooling-only annotation, no logic changed — same tradeoff already accepted
// elsewhere in this repo (e.g. farkle-engine's own stricter-than-consumer
// gap). Runtime behavior is unaffected; only compile-time checking here.

// Vendored from https://github.com/kevglass/propel-js (MIT License, see
// LICENSES/propel-js-LICENSE). Unmodified. "Simple serialisable deterministic
// 2D physics engine" — used here for purely decorative motion (die
// landing/settle juice, bomb knockback particles), never as a source of
// scoring truth. Bodies are cosmetic overlays on top of the server-dictated
// (row, col) a cell actually lands in (ADR-024/025).
//
// MINI 2D PHYSICS
// ===============
